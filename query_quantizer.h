...
void TransposeU8ToBitplanes(
    ...
    /* AVX2 fast path (b <= 8): process 32 codes per iteration, exact layout match. */
#if defined(SIMD_AVX_ENABLED) && defined(__AVX2__)
    if (b <= 8) {
        ...
        for (size_t blk = 0; blk < numBlocks; ++blk) {
            ...
            for (size_t offset = 0; offset < remain; offset += 32) {
                ...
                __m256i v = _mm256_loadu_si256((const __m256i*)tmp);
                ...
                for (size_t bit = 0; bit < b; ++bit) {
                    ...
                    // 将目标位移到每个字节的 MSB，然后用 movemask 压缩为 32bit mask
                    __m256i shifted = _mm256_slli_epi16(v, (int)(7 - bit));
                    unsigned mask = (unsigned)_mm256_movemask_epi8(shifted);
