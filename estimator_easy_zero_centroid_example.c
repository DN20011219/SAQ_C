...
{
    size_t numBits = 8;            // 保持 estimator_easy 的完整编码需求
    size_t dim = 4;                // 默认演示 4 维，质心固定为 (0,0,0,0)
    int data_count = 1000;
    int query_count = 10;
    int loop_count = 1;
    ...
    if (argc > 1) {
        ...
        if (data_count <= 0) data_count = 1000;
    }
    if (argc > 2) {
        query_count = atoi(argv[2]);
        if (query_count <= 0) query_count = 10;
    }
    if (argc > 3) {
    ...
    float *centroid = (float *)calloc(dim, sizeof(float)); // 固定 0 质心
    ...
    // Phase 1: 生成 data/query；直接使用 0 质心量化 data
    ...
    // Phase 2: 每个 query 构建 estimator_easy 上下文，估算距离
    for (int q = 0; q < query_count; ++q) {
